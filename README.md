...
│  └── user_summary       用户月度概况视图 (Athena VIEW，按自然月聚合)      │
└──────────┬──────────────────────────────────────────────────────────────┘
           │
           ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
| 成本分析 | credits | 每日超额趋势折线图、每日每用户 Credit 消耗趋势、各层级平均消耗柱状图、用户 Credit 使用明细表 |
| 用户概况 | summary | 总用户数 KPI、总活跃天数 KPI、每月用户 Credit 消耗柱状图、每月用户活跃天数柱状图、用户月度概况表（含层级变化追踪） |
...
| 3️⃣ | 通过 Glue API 创建外部表 + Athena 视图 | deploy.sh 内置 |
